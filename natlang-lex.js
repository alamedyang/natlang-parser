...
natlang.getTokenContext = function (inputString, token, message) {
	var pos = token.pos;
...
natlang.printParseMessageNaive = function (kind, line, row, col, message) {
	let arrow = '~'.repeat(col) + '^';
	line = line.replace(/\t/g,' ');
	let func;
	if(kind === "error") { func = console.error; }
	else if(kind === "warning") { func = console.warn; }
	else { func = console.log; };
	func(`╓ Line ${row}:${col}: ${message}`);
	func('║ ' + `${line}`);
	func('╙' + arrow);
};
natlang.printParseMessage = function (kind, inputString, pos, message) {
	let errorCoords = natlang.findLineAndCharNumbers(inputString, pos);
	natlang.printParseMessageNaive(
		kind, errorCoords.lineString, errorCoords.row, errorCoords.col,
		message
	);
};
